...
static int fdevent_select_event_next_fdndx(fdevents *ev, int ndx) {
    ...
    printf("the max_fd id is %d, init i is %d\n", ev->select_max_fd + 1, i);
...
#define SET(x) ev->x = fdevent_select_##x;
int fdevent_select_init(fdevents *ev) {
    perror("select_init_excuted1");
    ev->type = FDEVENT_HANDLER_SELECT;

    perror("select_init_excuted2");
    SET(reset);
    SET(poll);
