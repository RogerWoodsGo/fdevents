...
int fdevent_solaris_port_init(fdevents *ev){return 0;}
int fdevent_freebsd_kqueue_init(fdevents *ev){return 0;}
int fdevent_libev_init(fdevents *ev){return 0;}
...
fdevents *fdevent_init(size_t maxfds, fdevent_handler_t type) {
	...
	case FDEVENT_HANDLER_POLL:
		if (0 != fdevent_poll_init(ev)) {
            printf("event-handler poll failed\n");
	...
	case FDEVENT_HANDLER_SELECT:
		if (0 != fdevent_select_init(ev)) {
            printf("event-handler select failed\n");
			return NULL;
		}
		return ev;
	case FDEVENT_HANDLER_LINUX_SYSEPOLL:
		if (0 != fdevent_linux_sysepoll_init(ev)) {
            printf("event-handler linux-sysepoll failed\n");
			return NULL;
		}
		return ev;
	case FDEVENT_HANDLER_SOLARIS_DEVPOLL:
		if (0 != fdevent_solaris_devpoll_init(ev)) {
            printf("event-handler solaris-devpoll failed\n");
			return NULL;
		}
		return ev;
